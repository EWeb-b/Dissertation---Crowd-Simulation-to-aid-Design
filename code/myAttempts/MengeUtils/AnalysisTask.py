...
class AnalysisTask:
    ...
    def _parseConfigLine( self, file, name, convertFunc=None ):
        ...
        if ( tokens[0] != name ):
            ...
            # self.rsrc.logger.error( '\tRead: %s' % line )
            print( "Fail. Looking for key %s, found %s" % ( name, tokens[0] ) )
            print( '\tRead: %s' % line )

            raise ValueError, "Found wrong key value"
        if ( tokens[0] == name ):
            print( "Success. Looking for key %s, found %s" % ( name, tokens[0] ) )
            print( '\tRead: %s' % line )
        value = tokens[1]
        if ( convertFunc ):
            try:
    ...
    def execute( self ):
        ...
        if ( self.work ):
            ...
            if ( self.work & AnalysisTask.VIS ):
                ...
                visualizeGFS( frameSet.totalFrames(), reader, colorMap, imageName, self.outImgType, 1.0, None )
                print '\t\tdone in %.2f seconds' % ( time.clock() - s )
    ...
    def execute( self ):
        ...
        if ( self.work ):
            ...
            if ( self.work & AnalysisTask.VIS ):
                ...
                scbFileName = os.path.basename(self.scbName)
                Crowd.plotFlow( scbFileName, tempFile, frameSet.simStepSize, titlePrefix=self.workName, legendStr=names )
                print '\t\tdone in %.2f seconds' % ( time.clock() - s )
    ...
    def execute( self ):
        ...
        if ( self.work ):
            ...
            if ( self.work & AnalysisTask.VIS ):
                ...
                s=time.clock()
                scbFileName = os.path.basename(self.scbName)
                Crowd.plotFundDiag( tempFile, rects, scbFileName, names )
                print '\t\tdone in %.2f seconds' % ( time.clock() - s )
