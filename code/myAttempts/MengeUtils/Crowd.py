...
def plotFlow( scbFileName, outFileName, timeStep, titlePrefix='', legendStr=None, newFig=False, xlimits=None, ylimits=None ):
    ...
    plt.title( '%s %s' % (scbFileName, titlePrefix ))
...
##    plt.xlabel( 'Simulation time (s)' )
    ...
    plt.tight_layout()
    plt.savefig( outFileName + ".flow.png", dpi = 200 )
    plt.clf()
    dFile.close()
...
def framesInRegion( region, data ):
    ...
    # print (region.minCorner[0])
    # print(region.minCorner[1])
    # print(region.size[0])
    # print(region.size[1])
    while ( True ):
        try:
            ...
            if ( state ):
                ...
                if ( inRegion.has_key( simID ) ):
                    ...
                    if ( elapsed > agtTime[ simID, 1 ] ):
                        ...
                        distance[ simID ] = distSq
                        # print (agtTime[simID, :])
                        # print(np.sqrt(distance[simID]))
                        #if enter < 50: print(enter)

    distance = np.sqrt( distance )

    # for agt in data.agents:
    #     print("\nAgent state" + agt.state)
    # print("\nagentCount: ")
    # print(data.agentCount())
    # classes = data.getClasses()
    # print "\t%d classes:" % ( len( classes ) ), classes.keys()
    # print("SCB version: " + data.version)
    # print("hasStateData?: ")
    # print(data.hasStateData())
    # print("self.ids: ")
    # print(data.ids)
    # print (" framesInRegion agtTime array: ")
    # print(agtTime)
    # print ("\nframesInRegion distance array: ")
    # print(distance)
    # print(data)
    # for a in agtTime:
    #     if (a[0] == 0 and a[1] != 0): print(a)
...
def plotFundDiag( outFileName, rectDomains, scbFileName, names=None ):
    ...
    @param      rectDomains     A list of RectDomain instances.  Compute the fundamental diagram for
                                each agent who passes through the region.
    @param      names           A list of strings.  Names for the rectangular domains.
    @param      scbFileName     The name of the scb file. Used for the plot titles.
    '''
    if ( names is None ):
    ...
    for i, name in enumerate( fileNames ):
        ...
        if (fdData.size is not 0):
            regions.append( fdData )
            legendStr.append( displayNames[ i ] )

            plt.title( '%s Fundamental Diagram - %s' % (scbFileName, displayNames[ i ] ))
            plt.plot( fdData[:,0], fdData[:,1], '.', alpha=0.5 )
            m,b = plt.polyfit(fdData[:,0], fdData[:,1], 1)
            plt.plot(fdData[:,0], m*fdData[:,0] + b, label = 'gradient = %.3f' % m)
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig( figName + '_speed.png' )
            plt.clf()

            plt.title( '%s Fundamental Diagram - %s' % (scbFileName, displayNames[ i ] ))
            plt.plot( fdData[:,0], fdData[:,0] * fdData[:,1], '.', alpha=0.5 )
            m,b = plt.polyfit(fdData[:,0], fdData[:,0] * fdData[:,1], 1)
            plt.plot(fdData[:,0], m*fdData[:,0] + b, label = 'gradient = %.3f' % m)
            plt.xlabel( 'Density (people/m$^2$)' )
            ...
            figName, ext = os.path.splitext( fdFileName )
            plt.tight_layout()
            plt.savefig( figName + '_flow.png' )
            plt.clf()

    plt.title( '%s Fundamental Diagram - All Regions' % scbFileName)
    plt.xlabel( 'Density (people/m$^2$)' )
    plt.ylabel( 'Speed (m/s)' )
    plt.ylim( (0.0, 2.0) )
    if ( len( rectDomains ) > 0 ):
        ...
        plt.plot(x, m * x + b)
        legendStr.append('gradient = %.3f' % m)
        plt.legend( legendStr, loc='best' )
        plt.tight_layout()
        plt.savefig( outFileName + '_speed_all.png')
...
##    plt.ylim( (0.0, 2.0) )
    if ( len( rectDomains ) > 0 ):
        legendStr.pop()
        for data in regions:
            plt.plot( data[:,0], data[:,0] * data[:,1], '.', alpha=0.5 )
        m,b = plt.polyfit(x, x * y, 1)
        plt.plot(x, m*x + b, label = 'gradient = %.3f' % m)
        legendStr.append('gradient = %.3f' % m)
        plt.legend( legendStr, loc='best' )
        plt.tight_layout()
        plt.savefig( outFileName + '_flow_all.png')
    plt.clf()
