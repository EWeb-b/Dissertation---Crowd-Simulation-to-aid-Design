...
def visualizeGFS( totalFrames, gfsFile, cMap, outFileBase, imgFormat, mapRange=1.0, mapLimits=None, sites=None, obstacles=None ):
    '''Visualizes a grid file sequence with the given color map.

    @param      totalFrames     The total number of frames in the simulation.
    ...
    for grid, gridID in gfsFile:
        try:
            ...
            if ( sites is not None ):
                frame, frameID = sites.next()
            if (i % (int(totalFrames/10)) is 0 and i is not 0):
